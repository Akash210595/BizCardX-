...
# cur.execute("CREATE DATABASE IF NOT EXISTS BUSINESS_CARD")

engine = create_engine('mysql+pymysql://root:Akash21**@localhost/BUSINESS_CARD')  # use your MySql password
...
icon = Image.open("./Card images/1.png")
st.set_page_config(page_title= "BizCardX: Extracting Business Card Data with OCR ",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This OCR app is created by AKash"""})
...
if selected == "Home":
    ...
    st.write("## :green[Python,easy OCR, Streamlit, SQL, Pandas]")
    st.markdown("## :red[**Overview :**]")
    st.write("## :green[In this streamlit web app you can upload an image of a business card and extract relevant information from it using easyOCR. You can view, modify or delete the extracted data in this app. This app would also allow users to save the extracted information into a database along with the uploaded business card image. The database would be able to store multiple entries, each with its own business card image and extracted information]")
